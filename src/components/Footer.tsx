...
const socials: FooterLink[] = [
  {
    ...
    icon: <FontAwesomeIcon icon={faDiscord} className="w-3.5 h-3.5" />,
  },
  {
    label: "Instagram",
    href: "https://www.instagram.com/tamucybersec",
    icon: <FontAwesomeIcon icon={faInstagram} className="w-3.5 h-3.5" />,
  },
  {
    label: "LinkedIn",
    href: "https://www.linkedin.com/company/texas-a-m-cyber-security-club",
    icon: <FontAwesomeIcon icon={faLinkedin} className="w-3.5 h-3.5" />,
  },
  {
    label: "X",
    href: "https://x.com/TAMUCybersec",
    icon: <FontAwesomeIcon icon={faXTwitter} className="w-3.5 h-3.5" />,
  },
]
    ...
    <Container className="py-8 sm:py-12">
      <div className="flex flex-col lg:flex-row justify-between items-start gap-8 lg:gap-12">
        ...
        <div className="flex flex-col items-center lg:items-start gap-4 w-full lg:w-auto">
          <Logo size={60} className="sm:mb-2" />
          <Button
            asChild
            className="font-azonix w-full sm:w-[160px] h-9 text-sm bg-primary hover:bg-primary/90 rounded-md transition-colors"
          >
            <Link href="mailto:turn.on@java.script" className="flex items-center justify-center w-full h-full">
              Contact Us
            </Link>
          </Button>
        </div>
      ...
      <div className="text-center text-xs sm:text-sm text-muted-foreground mt-12 sm:mt-16">
        &copy; {new Date().getFullYear()} TAMU Cybersecurity Club. All rights reserved.
      </div>
    </Container>
  )
}
